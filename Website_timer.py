...
class WebsiteTimer:
    def __init__(self):
        ...
        self.website_var.set(self.website_list[0])
        ...
        self.add_website_button.pack(pady=5)
    ...
    def timer_loop(self):
        ...
        try:
            ...
            messagebox.showinfo("Website Timer", "Press OK when you are done.")
            end_time = time.time()
    ...
    def add_website(self):
        website = simpledialog.askstring("Add Website", "Enter a website URL:")
        if website:
            self.website_list.append(website)
            self.website_dropdown['menu'].delete(0, 'end')  # Clear the existing menu items
            
            for site in self.website_list:
                self.website_dropdown['menu'].add_command(label=site, command=tk._setit(self.website_var, site))
    
    def open_website(self):
        website = simpledialog.askstring("Open Website", "Enter a website URL:")
        if website:
            if not website.startswith("http://") and not website.startswith("https://"):
                website = "http://" + website
